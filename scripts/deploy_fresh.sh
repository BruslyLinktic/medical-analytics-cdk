#!/bin/bash
# Script para desplegar el sistema de analítica médica
# ENFOQUE COMPLETAMENTE NUEVO - Evita dependencias cíclicas

set -e  # Detener ejecución si algún comando falla

echo "======================================================="
echo "  DESPLIEGUE DEL SISTEMA DE ANALÍTICA MÉDICA"
echo "  (ENFOQUE NUEVO - SIN DEPENDENCIAS CÍCLICAS)"
echo "======================================================="
...
# Verificar existencia de bucket frontend
if aws s3 ls | grep -q "medical-analytics-frontend-dev"; then
  echo "⚠️  El bucket 'medical-analytics-frontend-dev' ya existe."
  read -p "¿Desea eliminarlo y crear uno nuevo? (s/n): " choice
  if [[ "$choice" == "s" || "$choice" == "S" ]]; then
    echo "Eliminando bucket frontend existente..."
    aws s3 rb s3://medical-analytics-frontend-dev --force
  else
    echo "Debe modificar el nombre del bucket frontend en cdn_stack.py antes de continuar."
    exit 1
  fi
fi
...
API_KEY=$(aws cloudformation describe-stacks --stack-name medical-analytics-ingestion-dev --query "Stacks[0].Outputs[?OutputKey=='ApiKeyOutput'].OutputValue" --output text)
